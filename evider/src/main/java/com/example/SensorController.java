...
public class SensorController {
     ...
     * varje sensorns listan, registrerar ett nytt simulerad värde för varje route. görs varje minut.
     *
     * @throws Exception
     */
    ...
    public void updateValue() throws Exception {
        try {
			...
			for(Sensor sensor : sensors) {
				repo.deleteSimulatedValue(sensor.getValues().get(0));
			}
			for(Sensor sensor: sensors) {
				...
				for(int routeId : routes) {
					...
					int newRegistredValue = (int) (value * factor);
					repo.registerValue(new RouteValueRegister(routeId, sensor.getId(), random.nextInt(newRegistredValue, newRegistredValue+30), LocalDateTime.now()));
				}
			}
        } catch (Exception ex) {
            System.out.println(ex.getMessage());
